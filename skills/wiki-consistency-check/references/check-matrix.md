...
- A value containing a `{{PLACEHOLDER}}` (e.g. `id: mii-ig-studie`) is
...
| Language (content) | `sushi-config.yaml` (`i18n-default-lang`), `input/fsh` | Namenskonventionen → Sprache | Resource `description`/`name`/`title` may be German, and then a translation extension is required on them? This module leads in German (`i18n-default-lang: de`, DE-first per meta-wiki Release-2027); report the setting, do not call another choice a deviation. | Module |
